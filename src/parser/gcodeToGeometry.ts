...
export class GcodeToGeometryConverter {
  ...
  private get scaledLineWidth(): number {
    return this.LINE_WIDTH * this.SCALE_FACTOR;
  }
  ...
  public convertGcodeToGeometry(gcodeContent: string, stats: GcodeStats): PrintGeometry {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (result) {
        ...
        if (newPosition.z > lastZ + 0.1 * this.SCALE_FACTOR) {
          // Scaled minimum layer height
          layerFromZ++;
          lastZ = newPosition.z;
  ...
  private redistributeGeometryByZ(
    layers: GeometryLayer[],
