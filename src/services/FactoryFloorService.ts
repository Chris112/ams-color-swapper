...
export class FactoryFloorService {
  ...
  constructor(scene: FactoryFloorScene, config: Partial<FactoryFloorConfig> = {}) {
  ...
  public async addPrint(
    ...
    try {
      ...
      throw new Error(
        `Failed to process G-code file: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private tryStartNextBuild(): void {
    ...
    const queuedPrint = Array.from(this.prints.values()).find(
      (p) => p.buildProgress === 0 && !p.isBuilding
    );
...
export class FactoryFloorStats {
  ...
  public getTotalFilamentUsed(): number {
    ...
    return prints.reduce((sum, print) => {
      return sum + (print.stats.filamentUsageStats?.total || 0);
    }, 0);
  }
