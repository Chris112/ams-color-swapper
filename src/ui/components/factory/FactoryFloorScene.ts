...
export class FactoryFloorScene {
  ...
  private findNextPosition(): THREE.Vector3 {
    ...
    for (
      let x = -this.FLOOR_SIZE / 2 + this.PRINT_SPACING;
      x < this.FLOOR_SIZE / 2;
      x += this.PRINT_SPACING
    ) {
      for (
        let z = -this.FLOOR_SIZE / 2 + this.PRINT_SPACING;
        z < this.FLOOR_SIZE / 2;
        z += this.PRINT_SPACING
      ) {
        const candidate = new THREE.Vector3(x, 0, z);
        const tooClose = positions.some((pos) => pos.distanceTo(candidate) < this.PRINT_SPACING);
