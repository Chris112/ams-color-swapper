...
export class PrintBuilder {
  ...
  public getProgress(): number {
    const progress =
      this.geometry.totalLayers > 0 ? (this.currentLayer + 1) / this.geometry.totalLayers : 0;
  ...
  private animate(): void {
    ...
    if (targetLayer > this.currentLayer) {
      ...
      this.eventEmitter.emit('layerComplete', {
  ...
  private updateLayerIndicator(layer: number): void {
    ...
    if (actualLayerHeight === 0) {
      ...
      const layerHeight =
        this.geometry.totalLayers > 1 ? totalHeight / this.geometry.totalLayers : 0.02;
...
export class PrintBuilderManager {
  ...
  public on(
    event: 'builderStateChanged',
    handler: (data: { id: string; state: BuildState }) => void
  ): void {
    this.eventEmitter.on(event, handler);
  }
