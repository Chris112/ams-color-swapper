...
export class VoxelDataStructure {
  ...
  constructor(
    ...
    this.dimensions = new Vector3(resolution.x * voxelSize, height, resolution.z * voxelSize);
  ...
  public getVoxelsInRadius(center: Vector3, radius: number): VoxelData[] {
    return Array.from(this.voxels.values()).filter(
      (voxel) => voxel.position.distanceTo(center) <= radius
    );
  }
